...
cname <- "E:/Sonali_MS/ITMD529/data_mining/Ford"
length(dir(cname))
dir(cname)
...
#ford:2006 05 2000-1994 parsing issues
...
for(file_index in length(dir(cname)):1){
  ...
  for(i in 1:length(tables)){
    p<-tables[[i]]
    for(j in 1:length(p)){
      if(sum(grepl("Principal", p[,j],ignore.case = TRUE))>0 && sum(grepl("Position", p[,j],ignore.case = TRUE))>0 && !is.null(p) && count ==0)
        count<-count+1
      if(sum(grepl("Year", p[,j],ignore.case = TRUE))>0 && !is.null(p) && count == 1)
        count<-count+1
      if(sum(grepl("Salary", p[,j],ignore.case = TRUE))>0 && !is.null(p) && count == 2)
        count<-count+1
    }
    if(count==3)
  ...
  garbage_value<-c("Â—","Â—Â—","Â Â Â Â","Â Â Â","Â—Â Â","ÂÂ","Â","$","â€‹","â€")
  ...
  if(sum(proxy.table[1:nrow(proxy.table),2] %in% year)==0 && names(proxy.table[2]) == "Name_and_Principal_Position")
    proxy.table[,2]<-NULL 
  if(sum(proxy.table[1:nrow(proxy.table),2] %in% year)==0 && names(proxy.table[2]) == "Year")
    proxy.table[,2]<-NULL
...
write.xlsx(x = final.summary.table, file = "E:/Sonali_MS/ITMD529/data_mining/dataset/Text_Mining.xlsx",
           sheetName = "Ford_Motor", row.names = FALSE)
